...
import { Users, Plus, UserPlus, Zap, UsersRound, ArrowRight, Check, X, Calendar } from 'lucide-react'
...
export default function MatchDayPage() {
  ...
  useEffect(() => {
    ...
    const checkMatch = async () => {
      ...
      if (matches?.length) {
        setExistingMatchId(matches[0].id)
      }
    }
    
    if (!authLoading) checkMatch()
  }, [authLoading])
  ...
  const handleCreateMatch = async () => {
    ...
    if (profile?.player_id && !setup.attending.includes(profile.player_id)) {
      setSetup(prev => ({ ...prev, attending: [...prev.attending, profile.player_id] }))
    }
