...
// Skeleton component for instant loading feel
function ProfileSkeleton() {
 return (
  <div className="min-h-screen pb-20">
   <main className="max-w-md mx-auto px-4 py-6 space-y-6">
    <div className="h-6 w-24 bg-white/10 rounded animate-pulse" />
    <div className="h-10 w-40 bg-white/10 rounded animate-pulse mx-auto" />
    
    <div className="glass rounded-2xl p-6 border border-white/10">
     <div className="w-20 h-20 rounded-full bg-white/10 animate-pulse mx-auto mb-3" />
     ...
     <div className="grid grid-cols-4 gap-3">
      {[1,2,3,4].map(i => (
       <div key={i} className="text-center">
        ...
        <div className="h-3 w-10 bg-white/10 rounded animate-pulse mx-auto mt-2" />
       </div>
      ))}
     </div>
    </div>
     ...
     <div className="grid grid-cols-3 gap-4">
      {[1,2,3,4,5,6].map(i => (
       <div key={i} className="text-center">
        ...
        <div className="h-3 w-14 bg-white/10 rounded animate-pulse mx-auto mt-2" />
       </div>
      ))}
     </div>
    </div>
...
export default function PublicProfilePage() {
 ...
 // Step 1: Immediately resolve player from constants (no DB call)
 useEffect(() => {
  if (!username) return
  ...
  // Instantly find player by name (no async needed)
  ...
  // Player found immediately - set it
  setPlayer(foundPlayer)
  
  // Step 2: Now load profile data and stats in parallel (after setting player)
  const loadData = async () => {
   try {
    // Load profile + all stats in parallel
    const [profileRes, matchesRes, statsRes, motmRes, ratingsRes] = await Promise.all([
     // Profile data
     supabase.from('user_profiles').select('*').eq('player_id', foundPlayer.id).single(),
     // All stats needed
    ...
    playerStats.forEach(s => {
     ...
     assists += s.assists || 0
     if (s.is_winner) wins++
     const pts = (s.goals || 0) * POINTS_SYSTEM.goal + (s.assists || 0) * POINTS_SYSTEM.assist
       + (s.is_winner ? POINTS_SYSTEM.matchWin : 0)
       + (s.clean_sheet ? POINTS_SYSTEM.cleanSheet : 0)
       + (s.played_as_gk && s.is_winner ? POINTS_SYSTEM.goalkeeperWinBonus : 0)
     coins += pts
    })
    ...
    if (playerRatings.length > 0) {
     ...
     const avgFwd = Math.round((playerRatings.reduce((a, r) => a + (r.forward_rating || 0), 0) / count) * 10) / 10
     const avgMid = Math.round((playerRatings.reduce((a, r) => a + (r.midfielder_rating || 0), 0) / count) * 10) / 10
     const avgDef = Math.round((playerRatings.reduce((a, r) => a + (r.defender_rating || 0), 0) / count) * 10) / 10
     const avgGk = Math.round((playerRatings.reduce((a, r) => a + (r.goalkeeper_rating || 0), 0) / count) * 10) / 10
     setRatings({ 
      forward: avgFwd, 
      midfielder: avgMid, 
      defender: avgDef, 
      goalkeeper: avgGk, 
      overall: Math.round((avgFwd + avgMid + avgDef + avgGk) / 4 * 10) / 10 
     })
    }
   } catch (err) {
